...
extern volatile uint8_t uvc_framebuffer0[UVC_MAX_FRAME_SIZE] __attribute__((section(".sram")));
extern uint32_t uvc_frame_length;
static USBH_StatusTypeDef USBH_VIDEO_InputStream(USBH_HandleTypeDef *phost) {
  ...
  switch (VIDEO_Handle->steam_in_state) {
    ...
    case VIDEO_STATE_DATA_IN:
      ...
      if ((result == USBH_URB_DONE) && ((phost->Timer - VIDEO_Handle->camera.timer) >= VIDEO_Handle->camera.Poll)) {
        ...
        uint32_t printf_len=32;
//        if (printf_len>rxlen) {
          printf_len=rxlen;
//        }
        for (uint32_t i = 0; i < printf_len; i++) {
          printf("%02x ", tmp_packet_framebuffer[i]);
        }
        printf("\r\n");
        HAL_Delay(200);
        video_stream_process_packet((uint16_t) rxlen);
        //        if (uvc_frame_length < UVC_MAX_FRAME_SIZE) {
        //          memcpy((void *) (uvc_framebuffer0 + uvc_frame_length), (void *) tmp_packet_framebuffer, rxlen);
        //          uvc_frame_length+=rxlen;
        //          printf("uvc frame recv:%u\r\n",uvc_frame_length);
        //        } else {
        //          printf("uvc frame size > UVC_MAX_FRAME_SIZE\r\n");
        //          uvc_frame_length = 0;
        //        }
